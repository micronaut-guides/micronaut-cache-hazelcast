We can verify that the cache works as expected with JUnit tests.
...
[source, java]
.src/test/java/micronaut/cache/hazelcast/service/MessageServiceTests.java
----
include::{sourceDir}/src/test/java/micronaut/cache/hazelcast/service/MessageServiceTests.java[tag=test]
}
----
...
For further testing of `@CachePut` and `@CacheInvalidate`, please see the `MessageServiceTests` in the /complete project.
